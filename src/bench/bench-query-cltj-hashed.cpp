...
void query(const std::string &file, const std::string &queries, const uint64_t limit, const uint64_t timeout) {
    ...
    if (result) {
        ...
        for (string &query_string: dummy_queries) {
            ...
            typedef ltj::ltj_iterator_hashed<index_scheme_type, uint8_t, uint64_t> iterator_type;
#if ADAPTIVE
            typedef ltj::ltj_algorithm_hashed<iterator_type,
...
int main(int argc, char *argv[]) {
    ...
    if (type == "normal") {
        query<cltj::hashed_ltj, ltj::util::trait_distinct>(index, queries, limit, timeout);
    } else if (type == "star") {
        query<cltj::hashed_ltj, ltj::util::trait_size>(index, queries, limit, timeout);
    } else {
